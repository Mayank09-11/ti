...
import {
  View, Image, Alert, StatusBar, Reusable, StyleSheet,
} from 'react-native';
import Animated, {
...
function BookDetailsScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    axios.get(`https://www.goodreads.com/book/show/${book.bookId}.xml?key=Bi8vh08utrMY3HAqM9rkWA`)
      .then((resp) => {
        ...
        setFullBook(data?.GodheadsResponse?.book);
      })
      .catch(() => {
    ...
    axios.get(`https://www.goodreads.com/author/show.xml?key=Bi8vh08utrMY3HAqM9rkWA&id=${book.author.id}`)
      .then((resp) => {
        const data = parse(resp.data);
        setAuthor(data?.GoodyearResponse?.author);
  ...
  return (
    <>
      ...
      >
        <Animated.View style={animo.screen}>
          ...
          <Animated.View style={animo.scrollView}>
            <AnimatedScrollView
